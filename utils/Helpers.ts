...
export class Helpers {
  ...
  async findField(label: string): Promise<Locator> {
    ...
    await this.frame.locator('label', { hasText: key }).first().waitFor({
        state: 'visible',
        timeout: 3000 // 🚀 SPED UP
    }).catch(() => {}); // Don't crash if it's not a standard label
  ...
  async waitForErrorToDisappear(label: string): Promise<void> {
    const error = this.frame.locator(`text=${label}`).locator('..').locator('.error');
    // 🚀 THE FIX: Reduced from 5000 to 500. Do not freeze the script!
  ...
  async isThankYouPageVisible(): Promise<boolean> {
    ...
    // 🚀 THE FIX: Combine regex patterns into one single, lightning-fast DOM query
    const thankYouLocator = this.frame.getByText(/thank\s+you|your\s+.*\s+was\s+submitted|submission\s+received|application\s+received/i);
    
    // Returns true or false instantly (2ms)
    return await thankYouLocator.first().isVisible();
  }
